...
(function(){
				...
				'<div class="menu-items">' + 
					'{{#elements}}' +
						'<div class="menu-item {{#selected}}selected{{/selected}}">' +
							'{{{label}}}{{#isSlider}}: <span>&lt; {{{sliderLabel}}} &gt;</span><input type="text" style="display: none;">{{/isSlider}}{{#isShortcut}}<span class="shortcut">{{{shortcut}}}</span>{{/isShortcut}}' +
						'</div>' +
					'{{/elements}}' +
				'</div>'+
			'</div>' +
	...
	ESX_MENU.render = function(){
		...
		if(typeof focused != 'undefined') {
			if (carhudShowed == true) {
				$('#menu_' + focused.namespace + '_' + focused.name).addClass('up')
			}
	...
	window.onData = (data) => {
		...
		switch(data.action){
			...
			case 'moveUP' : {
				if (ESX_MENU.getFocused()) {
					$('#menu_' + ESX_MENU.getFocused().namespace + '_' + ESX_MENU.getFocused().name).addClass('up')
					carhudShowed = true
				}
				break;
			}
			...
			case 'moveDOWN' : {
				if (ESX_MENU.getFocused()) {
					$('#menu_' + ESX_MENU.getFocused().namespace + '_' + ESX_MENU.getFocused().name).removeClass('up')
					carhudShowed = false
				}
				break;
			}
