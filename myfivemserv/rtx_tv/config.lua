...
Config.InterfaceColor = "#f99406" -- change interface color, color must be in hex
...
Config.TelevisionObjects = {
	...
	{ --  Armitage Games Club
		scale = vector3(0.0433, 0.0205, 1.0), -- rendertarget scale
		maxdistance = 100.0, --max distance for display screen--max distance for view screen
		maxcontroldistance = 50.0, -- max distance for control tv (its cannot be higher than maxdistance)
		rendertarget = false, -- enable render target
		nonobject = true,  -- enable this if you want use just screen, withount rendering on object
		nonobjectcoords = vector3(90.07, 15.56, 70.01), -- screen cordinates
		nonobjectrotation = vector3(0.0, 0.0, 290), -- screen rotation
	},	
...
Config.TelevisionObjectsLocations = { -- You need enable Config.TelevisionObjectPlacer for use this
	{
		...
		coords = vector3(121.50, -1041.78, 30.8), -- object coords
		rotation = vector3(0.0, 0.0, 250.0), -- object coords
		handler = nil,
	},
}
...
function Notify(text)
	...
	exports["mrg_ui"]:SendNotification({
		message = text,
		time = duration
	})
