...
local gtaInputMap = {
    ...
    INPUT_JUMP = "Spacja",
    INPUT_SPRINT = "Shift",
    INPUT_ATTACK = "LPM",     -- Lewy przycisk myszy
    INPUT_AIM = "PPM",        -- Prawy przycisk myszy
    INPUT_RELOAD = "R",
    INPUT_COVER = "Q",
    INPUT_MELEE_ATTACK_LIGHT = "LMB",
    INPUT_MELEE_ATTACK_HEAVY = "R",
    INPUT_MELEE_BLOCK = "Spacja",
    INPUT_OPEN_JOURNAL = "J"
}

_ShowNewHelpText = function(text)
    ShowHelp = true
    text = string.gsub(text, "{(.-)}", "<span class='button'>%1</span>")
    text = string.gsub(text, "~(INPUT_%w+)~", function(key)
        local mapped = gtaInputMap[key]
        if mapped then
            return "<span class='button'>" .. mapped .. "</span>"
        else
            return "<span class='button'>" .. key .. "</span>" -- fallback
        end
    end)
    message = text
end
