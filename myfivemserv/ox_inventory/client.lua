...
function client.openInventory(inv, data)
	...
	if canOpenInventory() then
		...
		if left then
			...
			-- WERSJA p_ambulancejob

			local damage = {
				{ id = "head", name = "head", count = 0 },
				{ id = "torso", name = "torso", count = 0 },
				{ id = "left_arm", name = "left_hand", count = 0 },
				{ id = "right_arm", name = "right_hand", count = 0 },
				{ id = "left_leg", name = "left_foot", count = 0 },
				{ id = "right_leg", name = "right_foot", count = 0 }
			}
			local damages = exports["p_ambulancejob"]:GetDamages()
			local state = plyState.damages or {}
			for _, v in pairs(state) do
				local part = damages[v.boneId].part
				for k, v2 in pairs(damage) do
					if v2.id == part then
						damage[k].count = damage[k].count + v.value
					end
				end
			end
			left.damage = damage
...
local function registerCommands()
	...
	lib.addKeybind({
		...
		onPressed = function()
			...
			damage = damage
			SendNUIMessage({ action = 'toggleHotbar', data = damage })
		end
	})
...
local function onEnterDrop(point)
	if not point.instance or point.instance == currentInstance and not point.entity then
		local model = GetDropModel(point.itemName, point.itemCount) or point.model or client.dropmodel
		local isWeapon = string.find(point.itemName, 'WEAPON_')
		local modelHash = type(model) == "string" and GetHashKey(model) or model

		if not (IsModelInCdimage(modelHash) and IsModelValid(modelHash)) then
			model = point.model or client.dropmodel
		end
		local isloaded = lib.requestModel(model, 2000)
		if not isloaded then
			model = point.model or client.dropmodel
		end
		RequestCollisionForModel(model)
		local entity = CreateObject(model, point.coords.x, point.coords.y, point.coords.z, false, true, false)
		SetModelAsNoLongerNeeded(model)
		PlaceObjectOnGroundProperly(entity)
		SetEntityCollision(entity, false, true)
		SetEntityCompletelyDisableCollision(entity, false, true)
		if point.rot then
			SetEntityHeading(entity, point.rot)
		end
...
local function createDrop(dropId, data)
	local i = 1
	while data.items[i] == nil do i = i + 1 end
...
dropItems = {
    ...
    weapon_knuckle = `prop_w_me_dagger`,
    ...
    weapon_combatpistol=`fzt_combat`,
    weapon_appistol=`fzt_appistol`,
    weapon_stungun=`fzt_taser`,
    weapon_pistol50=`fzt_pistol50`,
    weapon_snspistol=`fzt_pistolmk2`,
    money = `prop_anim_cash_pile_01`,
    
    -- WEED --
    wetweed = `bkr_prop_weed_drying_01a`,
    driedweed = `bkr_prop_weed_bud_02b`,
    dryingrack = `ep_weedrack_01_empty`,
    dryingrackadvanced = `ep_weedrack_02_empty`,
    weedbrick = `hei_prop_heist_weed_block_01b`,
    emptybag = `reh_prop_reh_bag_weed_01a`,
    fullbag = `sf_prop_sf_bag_weed_01a`,
    wateringcan = `prop_wateringcan`,
    scale = `v_ret_ml_scale`,
    joint = `p_cs_joint_02`,
}

GetDropModel = function(name,amount)
    if name == "money" then
        return GetMoneyModel(amount)
    end
	if name == "gold" then
        return GetGoldModel(amount)
    end
	if name == "coke_1g_packed" then
		return GetDropCoke(amount)
	end
    if dropItems[name] then
        return dropItems[name]
    elseif dropItems[name:lower()] then
        return dropItems[name:lower()]
    elseif Items[name] and Items[name].dropProp then
        return Items[name].dropProp
    elseif Items[name:lower()] and Items[name:lower()].dropProp then
        return Items[name:lower()].dropProp
    elseif string.find(name, "WEAPON_") and not IsDuplicityVersion() then
        return GetWeapontypeModel(name) or 910205311
    else
        return 910205311
    end
end
...
dropMoney = {
    ...
    [2] = `prop_anim_cash_pile_01`,
    [3] = `hei_prop_heist_cash_pile`,
    [4] = `bkr_prop_bkr_cash_roll_01`,
    [5] = `ex_prop_exec_cashpile`,
    [6] = `bkr_prop_bkr_cashpile_01`,
    [7] = `bkr_prop_bkr_cashpile_07`
}

GetMoneyModel = function(amount)
    local model = dropMoney[1]
    if amount > 999966 then   
        model = dropMoney[7]
    elseif amount > 49999 then
        model = dropMoney[6]
    elseif amount > 9999 then
        model = dropMoney[5]
    elseif amount > 4999 then
        model = dropMoney[4]
    elseif amount > 999 then
        model = dropMoney[3]
    elseif amount > 499 then
        model = dropMoney[2]
    end
    return model
end
dropGold = {
    [1] = `ch_prop_gold_bar_01a`,
    [2] = `hei_prop_heist_gold_bar`,
    [3] = `h4_prop_h4_gold_stack_01a`,
}

GetGoldModel = function(amount)
    local model = dropGold[1]
    if amount > 30 then
        model = dropGold[3]
...
dropCoke = {
    [1] = `prop_bzzz_drugs_coke001`,
    [2] = `prop_bzzz_drugs_coke002`,
    [3] = `prop_bzzz_drugs_coke003`,
}

GetDropCoke = function(amount)
    local model = dropCoke[1]
    if amount > 50 then
        model = dropCoke[3]
    elseif amount > 10 then
        model = dropCoke[2]
