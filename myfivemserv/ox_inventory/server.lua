...
lib.addCommand({'additem', 'giveitem'}, {
	...
	if item then
		...
		if server.loglevel > 0 then
			Utils.DiscordEmbed('admin', ('"%s" gave %sx %s to "%s"'):format(source.label, count, item.name, inventory.label), nil, nil, source.owner)
...
lib.addCommand('removeitem', {
	...
	if item and args.count > 0 then
		...
		if server.loglevel > 0 then
			Utils.DiscordEmbed('admin', ('"%s" removed %sx %s from "%s"'):format(source.label, args.count, item.name, inventory.label), nil, nil, source.owner)
		end
	end
end)
...
lib.addCommand('setitem', {
	...
	if item then
		...
		if source > 0 then
			exports['dbl_logs']:SendLog(source, string.format("/setitem %s %s %s %s", args.target, item.name, count, args.type), 'giveitem')
		end

		source = Inventory(source) or {label = 'console', owner = 'console'}

		if server.loglevel > 0 then
			Utils.DiscordEmbed('admin', ('"%s" set "%s" %s count to %sx'):format(source.label, inventory.label, item.name, args.count), nil, nil, source.owner)
