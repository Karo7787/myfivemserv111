...
openBuyMembership = function(gymTable)
    ...
    if Config.Menu == 'esx_menu_default' then
        ...
        for k, v in pairs(_gymTable.memberships) do
            if v.hours then
                elements3[#elements3 + 1] = {label = Config.Translate[Config.Language]['menu.element.membership_hours']:format(v.hours, v.price), hours = v.hours, price = v.price}
            else
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'select_membership', {
            ...
            if data3.current.hours then
                TriggerServerEvent('vms_gym:sv:acceptMembership', nil, _gymTable.requiredMembership, 0, data3.current.hours, data3.current.price)
                menu3.close()
            elseif data3.current.days then
                TriggerServerEvent('vms_gym:sv:acceptMembership', nil, _gymTable.requiredMembership, data3.current.days, 0, data3.current.price)
                menu3.close()
            end
    ...
    elseif Config.Menu == 'esx_context' then
        ...
        for k, v in pairs(_gymTable.memberships) do
            if v.hours then
                elements3[#elements3 + 1] = {title = Config.Translate[Config.Language]['menu.element.membership_hours']:format(v.hours, v.price), hours = v.hours, price = v.price}
            else
        ...
        ESX.OpenContext(Config.ESXContext_Align, elements3, function(menu, element)
            if element.hours then
                ESX.CloseContext()
                TriggerServerEvent('vms_gym:sv:acceptMembership', nil, _gymTable.requiredMembership, 0, element.hours, element.price)
            elseif element.days then
                ESX.CloseContext()
                TriggerServerEvent('vms_gym:sv:acceptMembership', nil, _gymTable.requiredMembership, element.days, 0, element.price)
            end
        end, function(menu)
        ...
        for _k, _v in pairs(_gymTable.memberships) do
            if _v.hours then
                elements3[#elements3 + 1] = {
                    header = Config.Translate[Config.Language]['menu.element.membership_hours']:format(_v.hours, _v.price), 
                    params = {
                        isAction = true,
                        event = function()
                            TriggerServerEvent('vms_gym:sv:acceptMembership', nil, _gymTable.requiredMembership, 0, _v.hours, _v.price)
                        end
                    }
                }
            elseif _v.days then
                elements3[#elements3 + 1] = {
                    ...
                    params = {
                        isAction = true,
                        event = function()
                            TriggerServerEvent('vms_gym:sv:acceptMembership', nil, _gymTable.requiredMembership, _v.days, 0, _v.price)
                        end
                    }
                }
            end
    ...
    elseif Config.Menu == 'ox_lib' then
        ...
        for _k, _v in pairs(_gymTable.memberships) do
            if _v.hours then
                elements3[#elements3 + 1] = {
                    title = Config.Translate[Config.Language]['menu.element.membership_hours']:format(_v.hours, _v.price), 
                    onSelect = function()
                        TriggerServerEvent('vms_gym:sv:acceptMembership', nil, _gymTable.requiredMembership, 0, _v.hours, _v.price)
                    end
                }
                ...
                if playersIsAround then
                    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'select_player', {
                        ...
                        if data2.current.playerid then
                            ...
                            for k, v in pairs(_gymTable.memberships) do
                                if v.hours then
                                    elements3[#elements3 + 1] = {label = Config.Translate[Config.Language]['menu.element.membership_hours']:format(v.hours, v.price), days = 0, hours = v.hours, price = v.price}
                                else
                                    elements3[#elements3 + 1] = {label = Config.Translate[Config.Language]['menu.element.membership']:format(v.days, v.price), days = v.days, hours = 0, price = v.price}
                                end
                            end
                            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'select_membership', {
                                title = Config.Translate[Config.Language]['menu.title.select_membership_time'], 
                                ...
                                if data3.current.days or data3.current.hours then
                                    TriggerServerEvent('vms_gym:sv:sendRequestOfMembership', playerId, _gymTable.requiredMembership, data3.current.days, data3.current.hours, data3.current.price)
                                    menu3.close()
                                    isMenuOpened = false
                                end
                            end, function(data3, menu3)
            ...
            if element.value == 'grant_membership' then
                ...
                if playersIsAround then
                    ESX.OpenContext(Config.ESXContext_Align, elements2, function(menu, element)
                        ...
                        for k, v in pairs(_gymTable.memberships) do
                            if v.hours then
                                elements3[#elements3 + 1] = {title = Config.Translate[Config.Language]['menu.element.membership_hours']:format(v.hours, v.price), days = 0, hours = v.hours, price = v.price}
                            else
                                elements3[#elements3 + 1] = {title = Config.Translate[Config.Language]['menu.element.membership']:format(v.days, v.price), days = v.days, hours = 0, price = v.price}
                            end
                        end
                        ESX.OpenContext(Config.ESXContext_Align, elements3, function(menu, element)
                            if element.days then
                                ESX.CloseContext()
                                TriggerServerEvent('vms_gym:sv:sendRequestOfMembership', playerId, _gymTable.requiredMembership, element.days, element.hours, element.price)
                                isMenuOpened = false
                            end
                        end, function(menu)
                            isMenuOpened = false
                                                ...
                                                if _v.hours then
                                                    elements3[#elements3 + 1] = {
                                                        ...
                                                        params = {
                                                            isAction = true,
                                                            event = function()
                                                                TriggerServerEvent('vms_gym:sv:sendRequestOfMembership', GetPlayerServerId(v), _gymTable.requiredMembership, 0, _v.hours, _v.price)
                                                            end
                                                        }
                                                    }
                                                else
                                                    elements3[#elements3 + 1] = {
                                                        ...
                                                        params = {
                                                            isAction = true,
                                                            event = function()
                                                                TriggerServerEvent('vms_gym:sv:sendRequestOfMembership', GetPlayerServerId(v), _gymTable.requiredMembership, _v.days, 0, _v.price)
                                                            end
                                                        }
                                                    }
                                                end
...
RegisterNetEvent("vms_gym:sv:ForceAddSkill", function(name, value)
    TriggerServerEvent('vms_gym:sv:addValue', name, value)
end)
...
RegisterNetEvent('vms_gym:cl:sendRequestOfMembership', function(sellerId, membershipName, days, hours, price)
    if Config.UseCustomQuestionMenu then
        Config.CustomQuestionMenu(sellerId, membershipName, days, hours, price)
    else
        local _sellerId, _membershipName = sellerId, membershipName
        if Config.Menu == 'esx_menu_default' then
            local elements = {}
            if hours ~= 0 then
                elements[#elements+1] = {label = Config.Translate[Config.Language]['menu.element.accept_membership_hours']:format(price, hours), hours = hours, price = price}
            else
                elements[#elements+1] = {label = Config.Translate[Config.Language]['menu.element.accept_membership']:format(price, days), days = days, price = price}
            end
            elements[#elements+1] = {label = Config.Translate[Config.Language]['menu.element.reject_membership']}
            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'buy_membership', {
                ...
                if data.current.hours then
                    TriggerServerEvent("vms_gym:sv:acceptMembership", _sellerId, _membershipName, 0, data.current.hours, data.current.price)
                    menu.close()
                elseif data.current.days then
                    TriggerServerEvent("vms_gym:sv:acceptMembership", _sellerId, _membershipName, data.current.days, 0, data.current.price)
                    menu.close()
                else
        ...
        elseif Config.Menu == 'esx_context' then
            ...
            if hours ~= 0 then
                elements[#elements+1] = {title = Config.Translate[Config.Language]['menu.element.accept_membership_hours']:format(price, hours), hours = hours, price = price}
            else
                elements[#elements+1] = {title = Config.Translate[Config.Language]['menu.element.accept_membership']:format(price, days), days = days, price = price}
            end
            elements[#elements+1] = {title = Config.Translate[Config.Language]['menu.element.reject_membership']}
            ESX.OpenContext(Config.ESXContext_Align, elements, function(menu, element)
                if element.hours then
                    TriggerServerEvent("vms_gym:sv:acceptMembership", _sellerId, _membershipName, 0, element.hours, element.price)
                    ESX.CloseContext()
                elseif element.days then
                    TriggerServerEvent("vms_gym:sv:acceptMembership", _sellerId, _membershipName, element.days, 0, element.price)
                    ESX.CloseContext()
                else
                    TriggerServerEvent("vms_gym:sv:rejectMembership", _sellerId)
                    ESX.CloseContext()
        ...
        elseif Config.Menu == 'qb-menu' then
            local elements = {
                {header = Config.Translate[Config.Language]['menu.title.buy_membership'], isMenuHeader = true},
                {
                    header = hours ~= 0 and Config.Translate[Config.Language]['menu.element.accept_membership_hours']:format(price, hours) or Config.Translate[Config.Language]['menu.element.accept_membership']:format(price, days),
                    params = {
                        isAction = true,
                        event = function()
                            if hours then
                                TriggerServerEvent("vms_gym:sv:acceptMembership", _sellerId, _membershipName, 0, hours, price)
                            else
                                TriggerServerEvent("vms_gym:sv:acceptMembership", _sellerId, _membershipName, days, 0, price)
                            end
                        end,
                    }
                },
        ...
        elseif Config.Menu == 'ox_lib' then
            lib.registerContext({
                ...
                options = {
                    {
                        title = hours ~= 0 and Config.Translate[Config.Language]['menu.element.accept_membership_hours']:format(price, hours) or Config.Translate[Config.Language]['menu.element.accept_membership']:format(price, days),
                        onSelect = function()
                            if hours then
                                TriggerServerEvent("vms_gym:sv:acceptMembership", _sellerId, _membershipName, 0, hours, price)
                            else
