...
$('.editphotobutton').click(function () {
    ...
    switch(whichedit) {
        ...
        case ' Kopiuj':
            ...
            selection.removeAllRanges();
            document.body.removeChild(node);
            $(this).html('<i class="fa-solid fa-clipboard-check"></i> Skopiowano').css('pointer-events', 'none')
            setTimeout(() => {
                $(this).html('<i class="fa-solid fa-clipboard"></i> Kopiuj').css('pointer-events', 'all')
            }, 1200);
            break;
        case ' Save':
