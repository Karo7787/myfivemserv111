...
if Config.UseCommand then
	RegisterCommand("housing", function()
		local PlayerData = ESX.GetPlayerData()
		if not PlayerData.dead and not LocalPlayer.state.isCuffed and not IsPauseMenuActive() then
			toggleUI(not UIOpen)
		end
	end, false)
end

RegisterNetEvent('mrg_realestate:client:toggleUI', function()
    local PlayerData = ESX.GetPlayerData()
    if not PlayerData.dead and not LocalPlayer.state.isCuffed and not IsPauseMenuActive() then
		toggleUI(not UIOpen)
	end
...
RegisterNUICallback("updatePropertyData", function(data, cb)
	...
	if changeType == 'UpdateShell' then
		local currentShells = exports['mrg_housing']:GetShells()
	...
	TriggerServerEvent("mrg_realestate:server:updateProperty", changeType, property_id, newData)
	cb("ok")
end)

RegisterNUICallback("addTenantToApartment", function(data, cb)
	TriggerServerEvent("mrg_realestate:server:addTenantToApartment", data)
	cb("ok")
end)

RegisterNUICallback("getNames", function(data, cb)
	if not data then return end
	local names = lib.callback.await("mrg_realestate:server:getNames",source, data)
	cb(names)
end)
...
RegisterNUICallback("startZonePlacement", function (data, cb)
	...
	if type == "UpdateGarage" then
        ...
        TriggerServerEvent("mrg_realestate:server:updateProperty", type, property_id, data)
	else
        data = {
            door = newData,
            street = street,
