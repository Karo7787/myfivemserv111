local PSHousing = exports['mrg_housing']
...
local function initialiseData()
	...
	SendNUIMessage({
		action = "setShells",
		data = exports['mrg_housing']:GetShells()
	})
...
CreateThread(function()
	if GetResourceState('mrg_housing') ~= 'started' then return end
	initialiseData()
end)

AddEventHandler('mrg_housing:client:initialisedProperties', initialiseData)

AddEventHandler('mrg_housing:client:updatedProperty', function(property_id)
	local property = exports['mrg_housing']:GetProperty(property_id)
...
AddEventHandler("mrg_housing:client:updateApartment", function(old, new)
...
RegisterNetEvent('mrg_housing:client:addProperty', function(propertyData)
