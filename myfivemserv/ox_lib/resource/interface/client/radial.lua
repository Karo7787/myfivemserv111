...
---@param id string?
---@param option number?
local function showRadial(id, option, active)
...
function lib.hideRadial()
    ...
    LocalPlayer.state.DisableChat = false
    lib.resetNuiFocus()
    table.wipe(menuHistory)
...
RegisterNUICallback('radialClick', function(index, cb)
    ...
    if item.menu then
        ...
        showRadial(item.menu, nil, index)
...
RegisterNUICallback('radialBack', function(_, cb)
    ...
    if lastMenu.id then
        return showRadial(lastMenu.id, lastMenu.option, 0)
    end
...
lib.addKeybind({
    ...
    onPressed = function()
        ...
        TriggerEvent("ox_lib:RadialOpened")
        LocalPlayer.state.DisableChat = true
        while isOpen do
