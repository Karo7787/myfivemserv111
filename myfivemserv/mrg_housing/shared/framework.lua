...
if IsDuplicityVersion() then
    ...
    function Framework.ox.Notify(src, message, type)
        ...
        TriggerClientEvent("ox_lib:notify", src, {title="Nieruchomość", description=message, type=type})
    end
...
Framework.ox = {
    ...
    AddApartmentEntrance = function (coords, size, heading, apartment, enter, seeAll, seeAllToRaid, _)        
        local handler = exports.ox_target:addBoxZone({
            ...
            options = {
                ...
                {
                    label = "Zobacz wszystkie apartamenty",
                    onSelect = seeAll,
                    icon = "fas fa-circle-info",
                },
                {
                    label = "Przeprowadź nalot",
    ...
    OpenInventory = function (stash, stashConfig, property_id)
