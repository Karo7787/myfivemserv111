...
function Apartment:EnterApartment()
    for propertyId, _  in pairs(self.apartments) do
        ...
        if property.owner then
            TriggerServerEvent('mrg_housing:server:enterProperty', propertyId)
            return
        end
    end

    Framework[Config.Notify].Notify("Nie masz tu mieszkania.", "error")
end

function Apartment:GetMenuForAll()

    if next(self.apartments) == nil then 
        Framework[Config.Notify].Notify("Nie ma tu żadnych apartamentów.", "error")
        return
    end
    ...
    for propertyId, _ in pairs(self.apartments) do
        ...
        if property then
            table.insert(menu.options,{
                ...
                description = 'SSN Właściciela #'..property.propertyData.owner_ssn,
                onSelect = function()
                    TriggerServerEvent('mrg_housing:server:enterProperty', propertyId) 
                end,
            })
...
function Apartment:GetMenuForAllToRaid()
    ...
    for propertyId, _ in pairs(self.apartments) do
        ...
        if property then
            table.insert(menu.options,{
                title = "Nalot " .. self.apartmentData.label .. " " .. propertyId,
                description = 'SSN Właściciela #'..property.propertyData.owner_ssn,
                onSelect = function()
                    TriggerServerEvent("mrg_housing:server:raidProperty", propertyId)
                end,
            })
        end
    end 
