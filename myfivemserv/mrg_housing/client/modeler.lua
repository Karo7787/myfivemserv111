...
Modeler = {
    ...
    UpdateFurniture = function (self, item)
        ...
        TriggerServerEvent("mrg_housing:server:updateFurniture", self.property_id, newFurniture)
    end,
    ...
    BuyCart = function (self)
        ...
        if not next(self.Cart) then
	        Framework[Config.Notify].Notify("Twój koszyk jest pusty", "error")
            return
        end
        ...
        if exports.ox_inventory:GetItemCount("money") < totalPrice then
	        Framework[Config.Notify].Notify("Nie masz wystarczająco dużo pieniędzy!", "error")
            return
        end
        local property = Property.Get(self.property_id)
        ...
        TriggerServerEvent("mrg_housing:server:buyFurniture", self.property_id, items, totalPrice, property.inGarden)
    ...
    RemoveOwnedItem = function (self, data)
        if data.type == 'storage' then
            ...
            if hasItems then Framework[Config.Notify].Notify('Skrytka nie jest pusta', 'error') return end
        end
        local item = data
        ...
        if item ~= nil then
            ...
            TriggerServerEvent("mrg_housing:server:removeFurniture", self.property_id, item.id)
        end
    end,
}
