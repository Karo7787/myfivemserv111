...
function Property:RegisterMlo()
    local data = lib.callback.await("mrg_housing:cb:getMainMloDoor", false, self.property_id, 1)
    ...
    -- this only for who have access or owning the house
        ...
        self.mloData.poly = lib.zones.poly({
            ...
            onEnter = function()
                TriggerServerEvent("mrg_housing:server:enterProperty", self.property_id,nil,true)
            end,
            onExit = function()
                self:LeaveShell()
            end
...
function Property:RegisterPropertyEntrance()
    local function enter()
        TriggerServerEvent("mrg_housing:server:enterProperty", self.property_id)
    end

    local function raid()
        TriggerServerEvent("mrg_housing:server:raidProperty", self.property_id)
    end

    local function showcase()
        TriggerServerEvent("mrg_housing:server:showcaseProperty", self.property_id)
    end

    local function showData()
        local data = lib.callback.await("mrg_housing:cb:getPropertyInfo", false, self.property_id)
        if not data then return end
    ...
    if not isMlo then
        ...
        local poly = lib.zones.poly({
            ...
            onEnter = function()
                self:GiveMenus(true)
                self.inGarden = true
                TriggerServerEvent("mrg_housing:server:enterGarden", self.property_id)
            end,
            onExit = function()
...
function Property:RegisterGarageZone()
    ...
    if isQbx then
        TriggerServerEvent('mrg_housing:server:qbxRegisterHouse', self.property_id)
    else
...
function Property:LeaveShell()
    ...
    TriggerServerEvent("mrg_housing:server:leaveProperty", self.property_id)
...
function Property:RemoveMenus(force)
    ...
    Framework[Config.Radial].RemoveRadialOption("house_menu")
end
...
function Property:ManageAccessMenu()
    ...
    if not self.owner then
        Framework[Config.Notify].Notify("Tylko właściciel może to zrobić.", "error")
        return
    end
...
function Property:GiveAccessMenu()
    ...
    local players = lib.callback.await("mrg_housing:cb:getPlayersInProperty", false, self.property_id) or {}
    ...
    if #players > 0 then
        for i = 1, #players do
            ...
            menu.options[#menu.options + 1] = {
                ...
                onSelect = function()
                    TriggerServerEvent("mrg_housing:server:addAccess", self.property_id, v.src)
                end,
            }
        end
        ...
        Framework[Config.Notify].Notify("Nikt nie przebywa na terenie nieruchomości", "error")
    end
end
...
function Property:RevokeAccessMenu()
    ...
    local playersWithAccess = lib.callback.await("mrg_housing:cb:getPlayersWithAccess", false, self.property_id) or {}
    ...
    if #playersWithAccess > 0 then
        for i = 1, #playersWithAccess do
            ...
            alreadyAccessMenu.options[#alreadyAccessMenu.options + 1] = {
                title = v.name,
                description = "Odbierz Dostęp",
                onSelect = function()
                    TriggerServerEvent("mrg_housing:server:removeAccess", self.property_id, v.citizenid)
                end,
            }
        end
        ...
        Framework[Config.Notify].Notify("Nikt nie ma dostępu do tej nieruchomości", "error")
    end
end
...
function Property:OpenDoorbellMenu()
    ...
    if not next(self.doorbellPool) then
        Framework[Config.Notify].Notify("Nikogo nie ma przy drzwiach", "error")
        return
    end
    ...
    for k, v in pairs(self.doorbellPool) do
        menu.options[#menu.options + 1] = {
            ...
            onSelect = function()
                TriggerServerEvent(
                    "mrg_housing:server:doorbellAnswer",
...
function Property:LoadFurniture(furniture)
    ...
    if furniture.type and Config.FurnitureTypes[furniture.type] then
        if not self.counttypes[furniture.type] then
            self.counttypes[furniture.type] = 0
        end
        self.counttypes[furniture.type] = self.counttypes[furniture.type] + 1
        Config.FurnitureTypes[furniture.type](entity, self.property_id, self.propertyData.shell, self.counttypes[furniture.type] == 1 and self.property_id or self.property_id..furniture.id)
    end
...
function Property:LoadFurnitures(isGarden)
    self.propertyData.furnitures = lib.callback.await('mrg_housing:cb:getFurnitures', false, self.property_id) or {}
...
function Property:RemoveProperty(doors)
    ...
    if doors then TriggerEvent('mrg_housing:client:DeleteOxDoors', self.property_id) end
    --@@ comeback to this
    -- Think it works now
...
function Property:UpdateApartment(newApartment)
    ...
    TriggerEvent("mrg_housing:client:updateApartment", oldAptName, newApartment)
end
...
RegisterNetEvent("mrg_housing:client:enterProperty", function(property_id, spawn)
    local property = Property.Get(property_id)
    if spawn == 'spawn' then
        local data = lib.callback.await("mrg_housing:cb:getMainMloDoor", false, property_id, 1)
...
RegisterNetEvent("mrg_housing:client:updateDoorbellPool", function(property_id, data)
    local property = Property.Get(property_id)
    property.doorbellPool = data
end)

RegisterNetEvent("mrg_housing:client:updateFurniture", function(property_id, furnitures, isGarden)
    local property = Property.Get(property_id)
...
RegisterNetEvent("mrg_housing:client:updateProperty", function(type, property_id, data)
    local property = Property.Get(property_id)
    ...
    TriggerEvent("mrg_housing:client:updatedProperty", property_id)
end)

RegisterNetEvent("mrg_housing:client:openFurnitureMenu", function(data)
    Modeler:OpenMenu(data.options.propertyId)
end)

RegisterNetEvent("mrg_housing:client:createOxDoors", function(data)
...
RegisterNetEvent("mrg_housing:client:DeleteOxDoors", function(propertyid)
    local name = ('ps_mloproperty%s'):format(propertyid)
    TriggerServerEvent('ox_doorlock:RemoveDoorlock', name)
end)

AddEventHandler("mrg_housing:client:openManagePropertyAccessMenu", function(data)
