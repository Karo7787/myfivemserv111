...
window.addEventListener("message", (event) => {
    ...
    switch(data.action) {
        case 'OpenRadio':
            ...
            break;
        case "updatecolor": 
            Root.style.setProperty('--main', String(data.color));
            break;
        case 'RadioTime':
...
const PlayMusic = ((link) => {
    ...
    radioData.playTime = 0;
    radioData.url = url;
    $.post('https://mrg_radiocar/PlayMusic', JSON.stringify({url: url, volume: volume}));
...
const StopMusic = (() => {
    ...
    antiSpam = nowDate + 500;
    $.post('https://mrg_radiocar/StopMusic');
    playBtn.onclick = (() => {PlayMusic()});
    playBtn.innerHTML = `<i class="fa-solid fa-play"></i>`;
    clearInterval(timeInterval);
    radioData.playTime = 0;
...
const ChangeVolume = ((type) => {
    ...
    if (type == 'up' && volume < 100) {
        ...
        if (radioData.url) {
            $.post('https://mrg_radiocar/ChangeVolume', JSON.stringify({volume: volume}));
        }
        return;
    }
...
const SaveMusic = (() => {
    ...
    musicSlots.forEach(slot => {
        ...
        slot.onclick = (() => {
            ...
            $.post('https://mrg_radiocar/SaveMusic', JSON.stringify({url: url, slot: number}));
        })
    })
})
...
const ToggleLoop = (() => {
    ...
    loopBtn.onclick = (() => {ToggleLoop(!radioData.loop)});
    $.post('https://mrg_radiocar/ToggleLoop', JSON.stringify({loop: radioData.loop}));
})
...
document.onkeydown = ((e) => {
    if (e.which == 27) {
        ...
        clearInterval(timeInterval);
        $('.radio').hide();
        $.post('https://mrg_radiocar/CloseRadio');
    }
})