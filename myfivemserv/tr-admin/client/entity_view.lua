local Relationshiptypes = {
	[1862763509] = 'PLAYER',
    [45677184] = 'CIVMALE',
    [1191392768] = 'CIVFEMALE',
    [-1533126372] = 'COP',
    [-183807561] = 'SECURITY_GUARD',
    [-1467815081] = 'PRIVATE_SECURITY',
    [-64182425] = 'FIREMAN',
    [1126561930] = 'GANG_1',
    [299800060] = 'GANG_2',
    [-1916596797] = 'GANG_9',
    [230631217] = 'GANG_10',
    [-1865950624] = 'AMBIENT_GANG_LOST',
    [296331235] = 'AMBIENT_GANG_MEXICAN',
    [1166638144] = 'AMBIENT_GANG_FAMILY',
    [-1033021910] = 'AMBIENT_GANG_BALLAS',
    [2037579709] = 'AMBIENT_GANG_MARABUNTE',
    [2017343592] = 'AMBIENT_GANG_CULT',
    [-1821475077] = 'AMBIENT_GANG_SALVA',
    [1782292358] = 'AMBIENT_GANG_WEICHENG',
    [-1285976420] = 'AMBIENT_GANG_HILLBILLY',
    [-2104069826] = 'DEALER',
    [-2065892691] = 'HATES_PLAYER',
    [-1072679431] = 'HEN',
    [2078959127] = 'WILD_ANIMAL',
    [580191176] = 'SHARK',
    [-837599880] = 'COUGAR',
    [-86095805] = 'NO_RELATIONSHIP',
    [-640645303] = 'SPECIAL',
    [-2143285144] = 'MISSION2',
    [1227432503] = 'MISSION3',
    [1531823744] = 'MISSION4',
...
local GetEntityInfo = function(entity)
    ...
    local entityData    = {
        '~y~'..locale("adminmenu.developer.entity_view_info"),
        '',
        locale("adminmenu.developer.model_hash")..' ~y~'..entityHash,
        ' ',
        locale("adminmenu.developer.ent_id")..' ~y~'..entity,
        locale("adminmenu.developer.obj_name")..' ~y~'.. entityName,
        locale("adminmenu.developer.net_id")..' ~y~'..(NetworkGetEntityIsNetworked(entity) and NetworkGetNetworkIdFromEntity(entity) or locale("adminmenu.developer.net_id_not_registered")),
        locale("adminmenu.developer.ent_owner")..' ~y~'..GetPlayerServerId(NetworkGetEntityOwner(entity)),
        ' '
    }

    if entityType == 1 then
        local pedRelationshipGroup = GetPedRelationshipGroupHash(entity)
        table.insert(entityData, locale("adminmenu.developer.cur_health")..' ~y~'..GetEntityHealth(entity))
        table.insert(entityData, locale("adminmenu.developer.max_health")..' ~y~'..GetPedMaxHealth(entity))
        table.insert(entityData, locale("adminmenu.developer.armour")..' ~y~'..GetPedArmour(entity))
        table.insert(entityData, locale("adminmenu.developer.rel_group")..' ~y~'.. (Relationshiptypes[pedRelationshipGroup] or locale("adminmenu.developer.rel_group_custom")))
        table.insert(entityData, locale("adminmenu.developer.rel_to_player")..' ~y~'..GetPedRelationshipType(GetRelationshipBetweenPeds(pedRelationshipGroup, PlayerPedId())))
    elseif entityType == 2 then
        table.insert(entityData, locale("adminmenu.developer.veh_rpm")..' ~y~'..RoundFloat(GetVehicleCurrentRpm(entity), 2))
        table.insert(entityData, (useKph and locale("adminmenu.developer.veh_speed_kph") or locale("adminmenu.developer.veh_speed_mph"))..' ~y~'..RoundFloat((GetEntitySpeed(entity)*(useKph and 3.6 or 2.23694)), 0))
        table.insert(entityData, locale("adminmenu.developer.veh_cur_gear")..' ~y~'..GetVehicleCurrentGear(entity))
        table.insert(entityData, locale("adminmenu.developer.veh_acceleration")..' ~y~'..RoundFloat(GetVehicleAcceleration(entity), 2))
        table.insert(entityData, locale("adminmenu.developer.body_health")..' ~y~'..GetVehicleBodyHealth(entity))
        table.insert(entityData, locale("adminmenu.developer.eng_health")..' ~y~'..GetVehicleEngineHealth(entity))
    ...
    table.insert(entityData, locale("adminmenu.developer.dist_to_obj")..' ~y~'.. RoundFloat(#(playerCoords-entityCoords), 2))
    table.insert(entityData, locale("adminmenu.developer.obj_heading")..' ~y~'.. RoundFloat(GetEntityHeading(entity), 2))
    table.insert(entityData, locale("adminmenu.developer.obj_coords")..' ~y~'.. RoundVector3(entityCoords, 2))
    table.insert(entityData, locale("adminmenu.developer.obj_rot")..' ~y~'.. RoundVector3(GetEntityRotation(entity), 2))
    table.insert(entityData, locale("adminmenu.developer.obj_velocity")..' ~y~'.. RoundVector3(GetEntityVelocity(entity), 2))
    ...
    Citizen.CreateThread(function()
        while EntityViewEnabled do
            ...
            if EntityFreeAim then
                DrawTitle("~y~"..locale("adminmenu.developer.entity_view_title").."~w~\n\n[~y~E~w~] - "..locale("adminmenu.developer.entity_freeaim_delete").."~w~\n[~y~G~w~] - "..locale("adminmenu.developer.entity_freeaim_freeze"), 0.15, 0.14)
                ...
                if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                    ...
                    if IsControlJustReleased(0, 47) then -- Freeze entities
                        ...
                        lib.notify({ title = locale("adminmenu.developer.you_have")..(FrozenEntities[entity] and locale("adminmenu.developer.entity_frozen") or locale("adminmenu.developer.entity_unfrozen")).. locale("adminmenu.developer.freeaim_entity"), description = locale("adminmenu.developer.you_have")..(FrozenEntities[entity] and locale("adminmenu.developer.entity_frozen") or locale("adminmenu.developer.entity_unfrozen")).. locale("adminmenu.developer.freeaim_entity")})
                    end
                    ...
                    if IsControlJustReleased(0, 38) then -- Delete entity
                        ...
                        if not DoesEntityExist(entity) then
                            lib.notify({
                                title =locale("adminmenu.developer.entity_del"),
								description =locale("adminmenu.developer.entity_del")
                            })
                        else
                            lib.notify({
                                title =locale("adminmenu.developer.entity_del_error"),
								description =locale("adminmenu.developer.entity_del_error")
                            })
                        end
                    end
                else
