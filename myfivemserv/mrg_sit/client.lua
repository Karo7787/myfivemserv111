...
local function CanPlayerReachSeat(destination, entity)
	...
	while true do
		...
		if GetEntityType(entityHit) ~= 1 then
			...
			if (dist < 1.0 or endCoords.x == 0.0) or entityHit == entity then
				return true
			else
				return false
			end
...
local function CanPlayerSitInSeat(coords, heading, entity, force)
	if force then
		return true
	end
...
local function SitOnSeat(data)
	...
	if not settings then
		--("^3Warning: No settings were set for type^2", seat.type, "^3 in Config.SitTypes, the default settings were used instead!")
		seat.type = 'default'
	...
	if data.entity ~= 0 and not CanPlayerSitInSeat(coords, heading, data.entity, seat.force or false) then
...
local function LayDownOnBed(data)
	...
	if Config.LayTypes[bed.type] then
		...
		--("^3Warning: No animation settings were set for type^2", bed.type, "^3 in Config.LayTypes, the default animation settings were used instead!")
...
local LocalText = {
	["sit_getup_keyboard"] = "Naciśnij <span class='button'>X</span> aby wstać",
	["sit_getup_controller"] = "Naciśnij <span class='button'>R2</span> aby wstać",
	["sit_on_keyboard"] = "Naciśnij <span class='button'>E</span> aby usiąść",
	["sit_down_controller"] = "Naciśnij <span class='button'>L2</span> aby usiąść",
	["lay_on_keyboard"] = "Naciśnij <span class='button'>G</span> aby się położyć",
	["lay_down_controller"] = "Naciśnij <span class='button'>L2</span> aby się położyć"
}

LocalText["both_on_keyboard"] = LocalText["sit_on_keyboard"].."<br/>"..LocalText["lay_on_keyboard"]
LocalText["both_down_controller"] = LocalText["sit_down_controller"].."<br/>"..LocalText["lay_down_controller"]
...
AddEventHandler('sit:helpTextThread', function(type)
	CreateThread(function()
		while metadata[type] do
			if IsUsingKeyboard(1) then
				ESX.ShowHelpNotification(LocalText["sit_getup_keyboard"])
			else
				ESX.ShowHelpNotification(LocalText["sit_getup_controller"])
			end
			Wait(0)
		end
	end)
