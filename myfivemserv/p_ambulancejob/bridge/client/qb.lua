...
Core.GetItemInfo = function(itemName)
    ...
    elseif GetResourceState('ps-inventory') == 'started' then
        return QBCore.Shared.Items[itemName]
    elseif GetResourceState('qs-inventory') == 'started' then
        local itemData = exports['qs-inventory']:GetItemList()[itemName]
        return itemData
    elseif GetResourceState('tgiann-inventory') == 'started' then
...
if GMT then
    ...
    AddEventHandler('onClientResourceStart', function(resourceName)
        ...
        Wait(1000)
        if not QBCore.PlayerData or not QBCore.PlayerData.job or not QBCore.PlayerData.job.name then
            return
        end
    
        if Config.Jobs[QBCore.PlayerData.job.name] then
            lib.addRadialItem({
                {
                  id = 'ambulance_menu',
...
AddEventHandler('piotreq_jobcore:UpdateDuty', function(data)
    if not Config.Jobs[QBCore.PlayerData.job.name] then
        return
    end
    ...
    if data.status == 1 then
        ...
        for channel, data in pairs(Config.Radio) do
            if data.jobs[QBCore.PlayerData.job.name] then
                items[#items + 1] = {
