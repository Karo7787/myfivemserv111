...
const UpdateRadio = ((channel, data) => {
    ...
    if (radioData[radioPage]) {
        for (const user of radioData[radioPage]) {
            ...
            <div class="radio-user" id="user-${user.player}">
                <span>${user.badge && user.badge != 0 ? `<div class='badge'>${user.badge}</div>` : ''}${user.name}</span>
    ...
    <div style="width: 100%;">CH ${channel}</div>
    <div>
...
window.addEventListener("message", (event) => {
    ...
    switch(data.action) {
        ...
        case 'LockRadar':
            ...
            break;
        case 'ToggleBodyCam':
            fetch("https://" + GetParentResourceName() + "/piotreq_bodycam", {
                method: "POST",
                headers: {
