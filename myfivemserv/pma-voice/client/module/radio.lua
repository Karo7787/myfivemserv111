		...
		if radioChannel > 0 then
			...
			if GetConvarInt('voice_enableRadioAnim', 0) == 1 and not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false)) and not disableRadioAnim then
				...
				elseif IsPedArmed(ped, 4) then
					...
					TaskPlayAnim(ped, "arrest", "radio_chatter", 8.0, 2.5, -1, 49, 0, 0, 0, 0)
				else
					...
					TaskPlayAnim(ped, "amb@code_human_police_investigate@idle_a", "idle_b", 8.0, 2.5, -1, 49, 0, 0, 0, 0)
				end
			end
...
RegisterCommand('-radiotalk', function()
	if (radioChannel > 0 or radioEnabled) and radioPressed then
		...
		if GetConvarInt('voice_enableRadioAnim', 0) == 1 then
			...
			StopAnimTask(ped, "arrest", "radio_chatter", -4.0)
			StopAnimTask(ped, "amb@code_human_police_investigate@idle_a", "idle_b", -4.0)
		end
...
RegisterCommand("radiovol", function(source, args, rawCommand)
	if tonumber(args[1]) >= 1 and tonumber(args[1]) < 101 then
		setVolume(tonumber(args[1]), "radio")
	end
end)
