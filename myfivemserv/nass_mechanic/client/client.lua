		...
		for _,v in pairs(Config.PedList) do
			if #(GetEntityCoords(plyped) - v.pos.xyz) < 20 then
				...
				if #(GetEntityCoords(plyped) - v.pos.xyz) < 1.9 then
					ShowHelpNotification("Nacisnij ~INPUT_CONTEXT~ to ask "..v.name.." by naprawic pojazd")
...
AddEventHandler('nass_mechanic:startvehrepair', function(data)
	if Config.checkForOnlineMechanic then
		ServerCallback('nass_mechanic:checkForMechanics', function(mechNum) 
			if mechNum > 0 then
				ShowNotification('Sa mechanicy w miescie, jedz do nich.')
			else
				verRepFunc(data)
			end
		end)
...
function verRepFunc(data)
	...
	if mechped == nil then
		ShowNotification(data.name ..' jest zajety daj mu pracowac.')
	else
...
function startCarRepair(mech)
	...
	if GetVehiclePedIsIn(playerPed, false) ~= 0 then
		ShowNotification('Wysiadz z pojazdu.')
	else
			...
			if #(engine - mech.pos.xyz) < 20 then
				...
				if Config.chargeForRepair then
					if getCash() >= repairCost then	
						ShowNotification(mech.name.." moze naprawic twoj pojazd.")
						if not exports.mrg_achivements:CheckIsUnlocked("usluga-lokalnego-mechanik") then
							TriggerServerEvent("mrg_achivements:UnlockAchivements", "usluga-lokalnego-mechanik", 1)
						end
						TriggerServerEvent("nass_mechanic:triggerServerSync", repairCost, mech, veh, engine)
					else
						ShowNotification('Nie masz pieniedzy. Naprawa kosztuje $'..repairCost..' ')
					end	
				else
				...
				ShowNotification('Auto jest za daleko.')
			end
		else
			ShowNotification('Nie znaleziono auta.')
		end
	end
end
