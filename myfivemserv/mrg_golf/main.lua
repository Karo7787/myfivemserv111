...
CreateThread(function()
	...
	while true do
		...
		local distance = #(playerLoc - vec3(-1332.78,128.18,56.03))
		...
		if distance < 30.0 then
			...
			ESX.DrawMarker(6, vec3(-1332.78,128.18,56.05), vec3(1.5, 1.5, 1.0), distance)
		end
		...
		if distance < 1.5 then
			sleep = false
			if golfplaying then
				ESX.ShowHelpNotification("Naciśnij ~g~ ~INPUT_CONTEXT~ ~s~ aby zakończyć grę w golfa.")
			else
				ESX.ShowHelpNotification("Naciśnij ~g~ ~INPUT_CONTEXT~ ~s~ aby zacząć grę w golfa.")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					spawnCart()
					startGolf()
					if not exports['mrg_achivements']:CheckIsUnlocked('golf') then
						TriggerServerEvent("mrg_achivements:UnlockAchivements", "golf", 1)
					end 
					ESX.ShowNotification("Przytrzymaj E aby uderzyć, A-D aby obrócić, Y aby zmienić kij.")
				end
			end
		end
...
function spawnCart()
	ESX.Game.SpawnVehicle('caddy', vector3(-1332.7823486328, 128.18229675293, 56.032329559326), 180, function(vehicle)
		...
		SetModelAsNoLongerNeeded(`caddy`)
		CurrentVeh = vehicle
...
function startGolfHud()
	while golfplaying do
		...
		if golfhole ~= 0 then
			local distance = math.ceil(#(GetEntityCoords(mygolfball) - vec3(holes[golfhole]["x2"],holes[golfhole]["y2"],holes[golfhole]["z2"])))
...
function startGolf()
	...
	CreateThread(function()
		while golfplaying do
			Citizen.Wait(1000)
			local playerLoc = GetEntityCoords(PlayerPedId())
			local distance = #(playerLoc - vec3(-1332.7823486328,128.18229675293,57.032329559326))
...
function MoveToBall()
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
			...
			local playerLoc = GetEntityCoords(PlayerPedId())
			local distance = #(playerLoc - ballLoc)
			if distance < 50.0 then
				ESX.ShowHelpNotification("Przejdź do swojej piłki, naciśnij ~g~E~s~ aby położyć nową piłkę jeśli utknąłeś.")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
...
function endShot()
	...
	local ballLoc = GetEntityCoords(mygolfball)
	local distance = #(ballLoc - vec3(holes[golfhole]["x2"],holes[golfhole]["y2"],holes[golfhole]["z2"]))
	if distance < 1.5 then
		ESX.ShowNotification("Piłka wpadła do dołka albo była w zasięgu dołka!")
		if not exports['mrg_achivements']:CheckIsUnlocked('golf-2') then
			TriggerServerEvent("mrg_achivements:UnlockAchivements", "golf-2", 1)
		end 
		if not exports['mrg_achivements']:CheckIsUnlocked('golf-3') then
			TriggerServerEvent("mrg_achivements:UnlockAchivements", "golf-3", 1)
		end 
	...
	if golfstrokes > 12 then
		ESX.ShowNotification("Zrobiłeś zbyt wiele uderzeń")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
...
function dropShot()
	...
	while doingdrop do
		...
		local distanceHole = #(vec3(holes[golfhole]["x2"],holes[golfhole]["y2"],holes[golfhole]["z2"]) - GetEntityCoords(PlayerPedId()))
		if distance < 50.0 and distanceHole > 50.0 then
			ESX.ShowHelpNotification("Naciśnij ~g~E~s~ aby upuścić tu piłkę.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			ESX.ShowHelpNotification("Naciśnij ~g~E~s~ aby upuścić nową piłkę - ~r~ jesteś za daleko od piłki lub za blisko dołka.")
		end
	end
end
...
function addBallBlip()
	...
	AddTextEntry('golf', 'Piłka')
	BeginTextCommandSetBlipName("golf")
...
function idleShot()
	...
	local distance = #(vec3(holes[golfhole]["x2"],holes[golfhole]["y2"],holes[golfhole]["z2"]) - GetEntityCoords(mygolfball))
	...
	while golfstate == 2 do
		...
		ESX.DrawMarker(6, vec3(holes[golfhole]["x2"],holes[golfhole]["y2"],holes[golfhole]["z2"]), vec3(0.5, 0.5, 1.0), false)
...
function BlipsStartEnd()
	...
	AddTextEntry('golf', 'Początek dołka')
	BeginTextCommandSetBlipName("golf")
	...
	AddTextEntry('golf', 'Koniec dołka')
	BeginTextCommandSetBlipName("golf")
...
function addblipGC()
	...
	AddTextEntry('golf', 'Pole golfowe')
	BeginTextCommandSetBlipName("golf")
...
AddEventHandler('customNotification', function(response)	
	TriggerEvent('chat:addMessage', {
		template = '<div class="chat-message"> <i class="far fa-info-circle" style="color:rgba(0, 191, 255, 1.0)"></i> <span class="chat-text"> &nbsp; {0}: {1} </span> </div>',
		args = { "Golf", response }
	})	
