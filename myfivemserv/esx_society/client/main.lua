		...
		if isBoss then
			...
			ESX.OpenContext("right", elements, function(menu,element)
				...
				elseif element.value == "withdraw_society_money" then
					local elements = {
						{unselectable = true, icon = "fas fa-wallet", title = TranslateCap('withdraw_amount'), description = "Wypłac środki z konta firmowego"},
						{icon = "fas fa-wallet", title = "Ilość", input = true, inputType = "number", inputPlaceholder = "Ilość do wypłaty...", inputMin = 1, inputMax = 2500000, name = "withdraw"},
						{icon = "fas fa-check", title = "Potwierź", value = "confirm"},
						{icon = "fas fa-arrow-left", title = "Wróć", value = "return"}
					}
				...
				elseif element.value == "deposit_money" then
					local elements = {
						{unselectable = true, icon = "fas fa-wallet", title = TranslateCap('deposit_amount'), description = "Zdeponuj środki do konta firmowego"},
						{icon = "fas fa-wallet", title = "Ilość", input = true, inputType = "number", inputPlaceholder = "Ilość do wpłacenia..", inputMin = 1, inputMax = 2500000, name = "deposit"},
						{icon = "fas fa-check", title = "Potwierź", value = "confirm2"},
						{icon = "fas fa-arrow-left", title = "Wróć", value = "return"}
					}
					ESX.RefreshContext(elements)
				...
				elseif element.value == "wash_money" then
					local elements = {
						...
						{icon = "fas fa-wallet", title = "Amount", input = true, inputType = "number", inputPlaceholder = "Amount to wash..", inputMin = 1, inputMax = 2500000, name = "wash"},
...
function OpenEmployeeList(society, options)
	ESX.TriggerServerCallback('esx_society:getEmployees', function(employees)
		local elements = {
			{unselectable = true, icon = "fas fa-user", title = "Pracownicy"}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element) 
			if element.value == "return" then
				...
				local elements2 = {
					...
					{icon = "fas fa-user", title = "Awansuj", value = "promote"},
					{icon = "fas fa-user", title = "Wyrzuć", value = "fire"},
					{icon = "fas fa-arrow-left", title = "Wróć", value = "return"}
				}
...
function OpenRecruitMenu(society, options)
	ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
		...
		elements[#elements+1] = {icon = "fas fa-arrow-left", title = "Wróć", value = "return"}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			if element.value == "return" then
				...
				local elements2 = {
					{unselectable = true, icon = "fas fa-user", title = "Potwierdź"},
