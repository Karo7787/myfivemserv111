...
---@return string
function GetJob()
    local esx_job = ESX.PlayerData?.job?.name or "unemployed"
    if esx_job == "unemployed" then
        local player_job = exports["sfjob"]:getActiveJob()
        if player_job ~= "unemployed" then
            esx_job = player_job
        end
    end
    return esx_job
...
function GetJobGrade()
    local esx_grade = ESX.PlayerData?.job?.grade or 0
    if esx_grade == "unemployed" then
        local player_job = exports["sfjob"]:getActiveJob()
        if player_job ~= "unemployed" then
            esx_grade = exports["sfjob"]:getActiveJobGrade()
        end
    end
    return esx_grade
end
