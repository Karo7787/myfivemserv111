...
local function connectToRadio(channel)
    ...
    if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(channel), ".")[2] ~= "" then
        Notify("Połączyłeś się z stacją: " .. channel .. " MHz", 'success')
    else
        Notify("Połączyłeś się z stacją: " .. channel .. " .00 MHz", 'success')
    end
end
...
function leaveRadio()
    ...
    Notify("Wyszedłeś z kanału", 'error')
end
...
RegisterNetEvent('mrg_radio:use:client', function()
    toggleRadio(not radioMenu)
end)

RegisterNetEvent('mrg_radio:onRadioDrop', function()
    if RadioChannel ~= 0 then
        leaveRadio()
    end
end)
...
Citizen.CreateThread(function()
    -- Volume
	local volumeData = GetResourceKvpString('volume-mrg_radio')
...
RegisterNUICallback('callback', function(data)
    ...
    elseif data.action == "volumeDown" then
        if RadioVolume > 1 then
            ...
            SetResourceKvp('volume-mrg_radio', RadioVolume)
        end
...
Citizen.CreateThread(function()
    for k, v in pairs(Config.RestrictedChannels) do
        RegisterNetEvent('mrg_radio:joinRadioChannel:client-' .. k, function()
...
RegisterCommand('RadioUp', function()
    local job = GetPlayerJob()
    if Config.FastSwitchJobs[job.name] and not IsPauseMenuActive() and not IsNuiFocused() then
        local rchannel = LocalPlayer.state.radioChannel
        if rchannel ~= 0 then
            local rchannel = LocalPlayer.state.radioChannel+1
            if Config.RestrictedChannels[rchannel] ~= nil then
                if CoreName == "es_extended" then
                    if Config.RestrictedChannels[rchannel][job.name] then
                        exports['pma-voice']:setRadioChannel(tonumber(rchannel))
                    else
                        TryConnectToUpperRadio(rchannel)
                    end
                end
            end
        end
...
TryConnectToUpperRadio = function(radio)
    local rchannel = radio+1
    if rchannel >= 1 then
        if Config.RestrictedChannels[rchannel] ~= nil then
            local job = GetPlayerJob()
            if CoreName == "es_extended" then
...
RegisterCommand('RadioDown', function()
    ...
    if Config.FastSwitchJobs[job.name] and not IsPauseMenuActive() and not IsNuiFocused() then
        local rchannel = LocalPlayer.state.radioChannel
        if rchannel ~= 0 and tonumber(rchannel)-1 ~= 0 then
            local rchannel = LocalPlayer.state.radioChannel-1
            if Config.RestrictedChannels[rchannel] ~= nil then
                local job = GetPlayerJob()
                if CoreName == "es_extended" then
                    if Config.RestrictedChannels[rchannel][job.name] then
                        ...
                        TryConnectToLowerRadio(rchannel)
                    end
                end
            end
        end
...
TryConnectToLowerRadio = function(radio)
    local rchannel = radio-1
    if rchannel >= 1 then
        if Config.RestrictedChannels[rchannel] ~= nil then
            local job = GetPlayerJob()
...
RegisterKeyMapping('RadioUp', "Przełącz stacje radiową (+)", 'keyboard', 'UP')
RegisterKeyMapping('RadioDown', "Przełącz stacje radiową (-)", 'keyboard', 'DOWN')