...
CreateThread(function()
    while true do
        if veh ~= 0 and player_is_emerg_driver then
            SendNUIMessage({
                action = "sirenControlHorn",
                state = (actv_horn and 1 or 0)
            })
            if state_lxsiren[veh] ~= 0 and IsVehicleSirenOn(veh) then
                SendNUIMessage({
            ...
            SendNUIMessage({
                ...
                state = (state_pwrcall[veh] == 2 and 1 or 0)
            })
            SendNUIMessage({
                action = "sirenControlManual",
                state = (actv_manu and 1 or 0)
            })
